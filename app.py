...
def token_obrigatorio(f):  # f de função
    @wraps(f)
    def decorated(*args, **kwargs):  # argumentos posicionais e nomeados
        token = None  # começa sem o token
        ...
        try:
            resultado = jwt.decode(
                # codigo algoritimo para decodificar o token
...
@app.route('/login')
def login():
    auth = request.authorization  # requer autorização user e senha
    # se nenhum estiver preenchido, retorna abaixo uma tela de login
    ...
    if auth.password == usuario.senha:
        token = jwt.encode({'id_autor': usuario.id_autor,
                            # o momento q o token foi gerado + 30 minutos para expirar
...
@app.route('/postages/<int:id_postagem>', methods=['DELETE'])
@token_obrigatorio
def excluir_postagem(autor, id_postagem):
    postagem_a_ser_excluida = Postagem.query._filter_by(
...
def obter_autores(autor):
    autores = Autor.query.all()  # extrair todos do banco de dados
    lista_de_autores = []  # sera incrementada/populada
...
# rodar flask após informações definidas
